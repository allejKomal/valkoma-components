...
import { Tooltip, TooltipContent, TooltipTrigger } from "valkoma-package/primitive"
...
export const ButtonGroup: React.FC<ButtonGroupProps> = ({
  ...
  const getButtonClasses = (index: number, isDisabled: boolean, isActive: boolean) => {
    const baseButtonClasses = cn(
      "flex items-center justify-center transition-colors duration-200 !text-[#575757] dark:!text-[#c5c7c5]",
    ...
    if (orientation === "horizontal") {
      if (items.length === 1) {
        return cn(baseButtonClasses, variant === "rounded" ? "rounded-full" : "rounded-lg")
      } else if (index === 0) {
        return cn(baseButtonClasses, variant === "rounded" ? "rounded-l-full" : "rounded-l-lg", "border-r border-gray-200")
      } else if (index === items.length - 1) {
        return cn(baseButtonClasses, variant === "rounded" ? "rounded-r-full" : "rounded-r-lg")
      } else {
      ...
      if (items.length === 1) {
        ...
        return cn(baseButtonClasses, variant === "rounded" ? "rounded-t-full" : "rounded-t-lg", "border-b border-gray-200")
      } else if (index === items.length - 1) {
        return cn(baseButtonClasses, variant === "rounded" ? "rounded-b-full" : "rounded-b-lg")
      } else {
    ...
    return (
      ...
      >
        ...
        {loading && loadingIconPosition === "start" && (
          <span className={cn("flex items-center justify-center", item.label && "mr-2")}>
        ...
        {!loading && item.icon && iconPosition === "start" && (
          <span className={cn("flex items-center justify-center", item.label && "mr-2")}>{item.icon}</span>
        )}

        {/* Label / Loading text */}
        {loading ? <span>{loadingText || "Loading..."}</span> : item.label && <span>{item.label}</span>}

        {/* Icon after label */}
        {!loading && item.icon && iconPosition === "end" && (
          <span className={cn("flex items-center justify-center", item.label && "ml-2")}>{item.icon}</span>
        )}

        {loading && loadingIconPosition === "end" && (
          <span className={cn("flex items-center justify-center", item.label && "ml-2")}>
            {loadingIcon || <LoaderCircle className="animate-spin" />}
          </span>
        )}
      </button>
